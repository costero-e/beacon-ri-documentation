...
export const useRegisterTextContent = (route: string, text: string, title: string) => {
  const { addSearchableContent, searchableContent } = useSearchableContent();

  useEffect(() => {
    const isAlreadyRegistered = searchableContent.some(content => content.route === route && content.text === text);
