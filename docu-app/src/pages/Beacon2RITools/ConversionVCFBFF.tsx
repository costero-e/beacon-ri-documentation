...
const ConversionVCFBFF = () => {
  ...
  return (
    <div className="conversionContainer">
      ...
      <p>
        Beacon RI Tools v2 will read the different columns for your variants in
        the VCF and place them inside the Legacy Variation schema for the Beacon
        v2 Spec.
      </p>
      <p>
        First of all, Beacon RI Tools v2 supports <b>VEP annotation</b>, which
      ...
      <p>
        Some of the fields will be parsed into BFF. Right now, the fields that
        will be read are:
      </p>
      <ul>
      ...
      <p>
        Additionally, for filling in the required fields, the INFO column will
      ...
      <ul>
        ...
        <p>AC_Hom → frequencyInPopulations|frequencies|alleleCountHomozygous</p>
        <p>
          AC_Het → frequencyInPopulations|frequencies|alleleCountHeterozygous
        </p>
        <p>
          END → variation|location|interval|end|value (in case END colum is not
          filled in)
        </p>
      </ul>
      ...
      <p>
        ...
        compressed and indexed (.vcf.gz + .vcf.gz.tbi). Beacon2 RI tools v2 will
        read all the VCF files inside the{" "}
        <a
      ...
      <p className="note">
        ...
        <div>
          If you are using the ri-tools image directly into Beacon v2 PI API or
          Beacon v2 RI API you won’t need to mongoexport the .json files unless
          you specifically need them for any other purpose, as the MongoDB
          instance for the API will already be filled with the variants.
        </div>
      </p>
      ...
      <p>
        After that, if needed, export your documents from the mongoDB to your
        machine as a BFF file (json) using two possible commands.
      </p>

      <ul>
        <li>
          The first command will delete an internal <i>"_id"</i> for each record
          that is generated by MongoDB:
          <div className="codeSnippet">
            <pre>
              <code>
        ...
        <li>
          ...
          MongoDB. Note that this ID is not part of the specifications of the
      ...
      <p>
        If you are converting with the paramater <b>case_level_data</b> to True,
        this will add data into two collections: <b>targets</b> and
        <b>caseLevelData</b>. If you need to export the variants to insert them
        in another mongoDB, you will need to export these two collections as
        well, by executing the next commands:
      </p>
      <div className="codeSnippet">
        <pre>
          <code>
      ...
      <p className="note">
        <img className="note-symbol" src="/note-symbol.png" alt="Note symbol" />
        <div>
          Bear in mind that if you are converting VCFs one by one for the same
          samples and not by batches (multiple VCFs at the same time), this will
          generate a target file every time. We recommend converting all VCFs
          for the same sampling (dataset) at the same time to avoid unnecessary
          target records to be created.
        </div>
      </p>
    </div>
  );
