...
const PiApiConfiguration = () => {
  ...
  return (
    <div className="apiConfigContainer">
      ...
      <p>
        ...
        conf for general configuration of the beacon and source for configuring
        the model of the beacon: e.g. entry types and the databases related to
        each entry type.
      </p>
      ...
      <p>
        ...
        conf.py files from beacon and deploy folders.
        <br />
      ...
      <p>
        There are 3 possible levels of beacon security for a dataset: public,
      ...
      <ul>
        ...
        <li>
          A controlled dataset is a dataset that needs a user to send a valid
          token for authentication and the user needs to be allowed to query
          that dataset.
        </li>
      </ul>
      <h4 id="method-1">Edit the .yml files inside permissions/datasets</h4>
      <p>
      ...
      <p>
        For making any IDP (based on OpenID and Oauthv2) work with beacon, you
        will need to add the <b>client ID </b>and <b>client secret</b> for each
        IDP in a new file you have to create inside auth/idp_providers folder
        (for each IDP). This file must have a name with an .env extension (e.g.
        something.env) and needs to have the following variables:
      </p>
      <div className="codeSnippet">
        <pre>
          <code>
      ...
      <h2 id="setting-default-granularity">Setting the default granularity</h2>
      <p>
        ...
        maximum, you can point it in <b>becon/conf/conf.py</b>, by editing the
        next variables:
      </p>
      <div className="codeSnippet">
        <pre>
      ...
      <p>
        ...
        file and you can write your handover there following the Beacon v2 spec
      ...
      <p>
        After doing that if you want to just show the handover in general (not
        per dataset) you will need to include it in the list_of_handovers array:
      </p>
      <div className="codeSnippet custom-codeSnippet">
        <pre>
      ...
      <p>
        ...
        create first a dictionary per dataset with the handover and the dataset
        id, like this:
      </p>{" "}
      <div className="codeSnippet custom-codeSnippet">
        <pre>
  ...
  "handover": handover_1
      ...
      <p>
        ...
        file called <b>manage.py</b> that you will need to edit in order to tell
        the API what is implemented in your beacon for each entry type and in
        what database you have the data related to each entry_type. <br />
        <br />
        In order to do this, you will just need to put a True or False response
        to what granularity you have implemented per each entryType. See the
        next example:
      </p>
      <div className="codeSnippet">
        <pre>
...
`}
      ...
      <p>
        If none of the granularities are True, then beacon will not look at the
        other variables (as entryType will be programmed as not implemented). If
        some of the granularities are True, then, singleEntryUrl will tell if a
        beacon has the id queries implemented for this entryType and for each
        endpoint inside endpoints, which cross query with this entryType and the
        id parameter is implemented as well. The <b>testMode</b> is to point if
        the entryType can be queried using testMode and the database field will
        tell the users which database has the data for that particular
        entryType.
      </p>
      <p className="note">
        <img className="note-symbol" src="/note-symbol.png" alt="Note symbol" />
        <div>
          Bear in mind that the name of the database variable needs to be the
          exact same name that the name for the folder it corresponds to in
          beacon/connections.
        </div>
      </p>
      <h2 id="setting-logs">Setting the logs</h2>
      <p>
        In this production instance of beacon, you can set the level of the logs
        you want to output and where do you want to see the output in. <br />
        <br />
        All the outputs will be streamed but if you define a path for a log file
        (file with extension .log) you will be able to have all the history of
        logs for your beacon saved in this file. <br />
        <br />
        For setting the level of the logs, specify one amongst NOTSET, DEBUG,
        ERROR, INFO in the variable level (after logging.):
      </p>
      <div className="codeSnippet">
        <pre>
      ...
      <p className="note">
        <img className="note-symbol" src="/note-symbol.png" alt="Note symbol" />
        <div>
          To save the logs in a file out of docker, you will need to create a
          volume for the file. There is a default log file volume defined in the
          docker-compose, which is -
          <b>./beacon/logs/logs.log:/beacon/logs/logs.log </b>, hence no need to
          create any other volume if you use this same file for logs.
        </div>
      </p>
      <h2 id="TLS-configuration">TLS configuration</h2>
      <p>
        To enable TLS for the Becaon API set beacon_server_crt and
        beacon_server_key to the full paht of the server certificate and server
        key in <b>beacon/conf/conf.py</b> file.
      </p>
      <h2 id="TLS-secured-MongoDB">TLS secured MongoDB</h2>
      <p>
        Edit the file <b>beacon/connections/mongo/conf.py </b> and
        set database_certificate to the full path to the client certificate. If
        a private CA is used also set the database_cafile to the full path to
        the CA certificate.
        <li>
